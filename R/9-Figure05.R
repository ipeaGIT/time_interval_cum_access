...
acc <- function(cutoff, tmin,tmax){
  ...
  access_cutoff <- dplyr::left_join(b,access,by=c("id_hex"="fromId")) %>%
    filter(!is.na(cenario))
  
  access_interval <- dplyr::left_join(b,access_mean,by=c("id_hex"="fromId")) %>%
    filter(!is.na(cenario))
...
ggsave2(filename="Fig_5.png", plot=ggplot2::last_plot(),
